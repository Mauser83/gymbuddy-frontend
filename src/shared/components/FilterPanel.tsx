...
function DirectionChevron({
  ...
  return (
    <Text style={{ color: theme.colors.accentStart, width: 20, textAlign: 'center' }}>
      {direction === 'right' ? (expanded ? '◀' : '▶') : expanded ? '▶' : '◀'}
    </Text>
  );
}

export default function FilterPanel({ options, onChangeFilters }: FilterPanelProps) {
  const { theme } = useTheme();
  ...
  const [parentDirections, setParentDirections] = useState<Record<string, 'left' | 'right'>>({});
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>
        {Object.entries(selectedFilters).flatMap(([key, values]) =>
          values.map((value) => (
            ...
            >
              <Text style={{ color: theme.colors.textPrimary, marginRight: 4 }}>{value}</Text>
      ...
      <Portal>
        {activeCategory && (
          <>
            ...
            >
              {isNested(options[activeCategory].options) ? (
                <ScrollView
                  ...
                  nestedScrollEnabled
                >
                  {Object.keys(options[activeCategory].options).map((parent, index, arr) => {
                    ...
                    return (
                      ...
                      >
                        {direction === 'left' && (
                          <DirectionChevron direction="left" expanded={activeParent === parent} />
                        )}
                        <Text
                          style={{
                            flex: 1,
                            color: theme.colors.textPrimary,
                            fontWeight: activeParent === parent ? 'bold' : 'normal',
                          }}
                        >
                          {parent}
                        </Text>
                        {direction === 'right' && (
                          <DirectionChevron direction="right" expanded={activeParent === parent} />
                        )}
                      </Pressable>
                    );
                  })}
                ...
                <ScrollView
                  ...
                  nestedScrollEnabled
                >
                  {(options[activeCategory].options as string[]).map((option, index) => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={{
                          ...
                          borderBottomColor: theme.colors.textSecondary,
                          borderBottomWidth:
                            index !== (options[activeCategory].options as string[]).length - 1
                              ? 1
                              : 0,
                          backgroundColor: theme.colors.surface,
                        }}
            ...
            {/* Sub Dropdown */}
            {activeParent && activeCategory && isNested(options[activeCategory].options) && (
              <Animated.View
                style={{
                  position: 'absolute',
                  top: dropdownPos.y + 4,
              ...
              >
                {(options[activeCategory].options as Record<string, string[]>)[activeParent].map(
                  (option: string, index: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Text style={{ color: theme.colors.textPrimary, flex: 1 }}>{option}</Text>
                      </Pressable>
                    );
                  },
                )}
